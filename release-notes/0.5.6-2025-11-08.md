...
- Database migration creates `maintenance_settings` table (migration: `20251108_205148_v0_5_6`)
...
**Benefits**:
- ✅ Controlled access during maintenance periods
...
This update requires a database migration to add the maintenance settings table. The migration is **backward compatible** and will not affect existing functionality.
...
1. Run the migration command:
   ...
   ```
2. This will create the `maintenance_settings` table with default values (`maintenanceMode: false`)
3. The migration is non-breaking and preserves all existing data

**What the Migration Does**:
- Creates `maintenance_settings` table with fields:
...
- ✅ Job history is automatically available
- ✅ No configuration changes needed
- ✅ Maintenance mode can be enabled/disabled via admin interface
...
**Maintenance Mode**:
- Maintenance mode can be enabled/disabled via Admin → Server → Maintenance mode
- Maintenance mode defaults to disabled
...
- Created database migration `20251108_205148_v0_5_6` for `maintenance_settings` table

...
describe("Validate Script Tag", () => {
    ...
    test("should validate Umami script tag with data-website-id", () => {
        ...
        if (result.ok) {
            ...
            expect(result.value.defer).toBe(true);
            expect(result.value["data-website-id"]).toBe("63b7582a-1ce5-46fd-8635-612cbba6cd1c");
        }
    });
    ...
    test("should validate Google Analytics script tag", () => {
        ...
        expect(result.ok).toBe(true);
        if (result.ok) {
            expect(result.value.src).toBe("https://www.googletagmanager.com/gtag/js?id=G-XXXXXXXXXX");
            expect(result.value.async).toBe(true);
        }
    });
    ...
    test("should reject inline script content", () => {
        ...
        expect(result.ok).toBe(false);
        if (!result.ok) {
            expect(result.error.message).toContain("Inline script content is not allowed");
        }
    });
    ...
    test("should handle script tag with URL containing query parameters", () => {
        ...
        expect(result.ok).toBe(true);
        if (result.ok) {
            expect(result.value.src).toBe("https://example.com/script.js?v=1.0&id=123");
        }
    });

		...
		try {
			...
			if (contentOrder === 0) {
				...
				if (parentSectionId) {
					...
					const maxSectionContentOrder =
						existingSections.docs.length > 0
							? existingSections.docs[0].contentOrder
							: 0;
					const maxModuleContentOrder =
						existingModules.docs.length > 0
							? existingModules.docs[0].contentOrder
							: 0;
					contentOrder =
						Math.max(maxSectionContentOrder, maxModuleContentOrder) + 1;
				} else {
					// Root level section
					...
					contentOrder =
						rootSections.docs.length > 0
							? rootSections.docs[0].contentOrder + 1
							: 1;
				}
			}
...
export const tryNestSection = Result.wrap(
	async (args: NestSectionArgs) => {
		...
		try {
			...
			const newOrder =
				siblings.docs.length > 0 ? siblings.docs[0].order + 1 : 1;
...
export const tryUnnestSection = Result.wrap(
	async (args: UnnestSectionArgs) => {
		...
		try {
			...
			const newOrder =
				rootSections.docs.length > 0 ? rootSections.docs[0].order + 1 : 1;
	...
	async (args: AddActivityModuleToSectionArgs) => {
		...
		try {
			...
			if (linkOrder === 0) {
				...
				linkOrder =
					existingModules.docs.length > 0
						? (existingModules.docs[0].order ?? 0) + 1
						: 1;
			}
	...
	async (args: GeneralMoveArgs) => {
		...
		try {
			...
			if (location === "inside") {
				...
				if (isRootMove) {
					// Moving to root level - get max order among root sections
					const rootSections = await payload.find({
						collection: CourseSections.slug,
						where: {
							course: { equals: sourceCourseId },
							parentSection: { equals: null },
						},
						sort: "-order",
						limit: 1,
						user,
					...
					// Moving inside a target section - get max contentOrder in target section
					...
					const maxSectionContentOrder =
						targetContent.docs.length > 0
							? targetContent.docs[0].contentOrder
							: 0;
					const maxModuleContentOrder =
						targetActivityModules.docs.length > 0
							? targetActivityModules.docs[0].contentOrder
							: 0;
					newContentOrder =
						Math.max(maxSectionContentOrder, maxModuleContentOrder) + 1;
			...
			if (source.type === "section") {
				...
				if (newParentSectionId) {
					...
					newOrder =
						siblingSections.docs.length > 0
							? siblingSections.docs[0].order + 1
							: 1;
				} else {

...
describe("Activity Module Management Functions", () => {
    ...
    beforeAll(async () => {
        ...
        const instructorResult = await tryCreateUser(payload, mockRequest, instructorArgs);
        const studentResult = await tryCreateUser(payload, mockRequest, studentArgs);
    ...
    test("should create a new activity module with initial version", async () => {
        ...
        const result = await tryCreateActivityModule(payload, mockRequest, createArgs);
        ...
        if (result.ok) {
            ...
            expect(result.value.commit.message).toBe("Initial creation of test page module");
    ...
    test("should update an activity module and create new version", async () => {
        ...
        expect(result.ok).toBe(true);
        if (result.ok) {
            expect(result.value.activityModule.title).toBe("Updated Test Page Module");
            ...
            const commits = allVersions.docs.map(version => {
                ...
                throw new Error('Commit should be populated with hash, message, author, and commitDate');
            });
            ...
            const initialCommitData = {
                content: JSON.stringify(initialContent, Object.keys(initialContent).sort()),
            ...
            const updatedCommitData = {
                content: JSON.stringify(updatedContent, Object.keys(updatedContent).sort()),
            ...
            const isBadValid = updatedTree.verify(badProof, badLeaf, updatedExpectedHash);

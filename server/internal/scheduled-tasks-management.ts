 ...
 */
export const tryGetScheduledTasks = Result.wrap(
	async (payload: Payload): Promise<GetScheduledTasksResult> => {
 ...
 */
export const tryGetPendingScheduledTasks = Result.wrap(
	async (payload: Payload): Promise<ScheduledTaskInfo[]> => {
		const drizzle = payload.db.drizzle;
		const now = new Date();

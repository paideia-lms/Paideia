...
export const tryCreateWhiteboard = Result.wrap(
    async (args: CreateWhiteboardArgs) => {
        const { payload, content, userId, user = null, req, overrideAccess = false } = args;
    ...
    (error) => transformError(error) ?? new UnknownError("Failed to create whiteboard"),
);

export const tryUpdateWhiteboard = Result.wrap(
    async (args: UpdateWhiteboardArgs) => {
        const { payload, id, content, user = null, req, overrideAccess = false } = args;
    ...
    (error) => transformError(error) ?? new UnknownError("Failed to update whiteboard"),
);
...
export const tryDeleteWhiteboard = Result.wrap(
    ...
    (error) => transformError(error) ?? new UnknownError("Failed to delete whiteboard"),
);
...
export const tryGetWhiteboardById = Result.wrap(
    ...
    (error) => transformError(error) ?? new UnknownError("Failed to get whiteboard by ID"),
);

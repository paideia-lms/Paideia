...
import { tryFindGrantsByActivityModule, tryFindInstructorsForActivityModule } from "../internal/activity-module-access";
...
export const tryGetUserModuleContext = Result.wrap(
    async (
        ...
        const transformedModule = {
            ...
            assignment:
                module.type === "assignment" && typeof module.assignment === "object" && module.assignment !== null
                    ? {
                        ...
                        allowLateSubmissions: module.assignment.allowLateSubmissions || null,
                    ...
                    : null,
            quiz:
                module.type === "quiz" && typeof module.quiz === "object" && module.quiz !== null
                    ? {
                        id: module.quiz.id,
                    ...
                    : null,
            discussion:
                module.type === "discussion" && typeof module.discussion === "object" && module.discussion !== null
                    ? {
        ...
        // unique by course id 
        const uniqueCourses = linksResult.value.map(link => link.course).filter((course, index, self) => self.findIndex(c => c.id === course.id) === index).map(course => ({
            id: course.id,
            title: course.title,
            slug: course.slug,
    ...
    (error) => transformError(error) ?? new UnknownError("Failed to get user module context", { cause: error }),
);

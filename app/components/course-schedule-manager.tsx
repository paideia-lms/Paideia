...
export function CourseScheduleManager({
	...
	const calendarEvents: EventInput[] = useMemo(() => {
		...
		const recurring = recurringSchedules?.map((item) => ({
		...
		for (let recurringIndex = 0; recurringIndex < recurring.length; recurringIndex++) {
			const recurringItem = recurring[recurringIndex];
			if (!recurringItem) continue;

			const startDate = recurringItem.startDate ? dayjs(recurringItem.startDate) : now;
			const recurrenceEnd = recurringItem.endDate ? dayjs(recurringItem.endDate) : endDate;
			const actualEnd = recurrenceEnd.isBefore(endDate) ? recurrenceEnd : endDate;

			for (let date = startDate; date.isBefore(actualEnd) || date.isSame(actualEnd, "day"); date = date.add(1, "day")) {
				const dayOfWeek = date.day();
				if (recurringItem.daysOfWeek.includes(dayOfWeek)) {
					const dateStr = date.format("YYYY-MM-DD");
					// Create Date objects for FullCalendar
					const startDateTime = dayjs(`${dateStr}T${recurringItem.startTime}:00`).toDate();
					const endDateTime = dayjs(`${dateStr}T${recurringItem.endTime}:00`).toDate();
		...
		// Add specific dates
		const specific = specificDates?.map((item) => ({
			date: item.date ? dayjs(item.date).format("YYYY-MM-DD") : "",
			startTime: item.startTime ?? "09:00",
			endTime: item.endTime ?? "12:00",
		})) ?? [];

		for (let specificIndex = 0; specificIndex < specific.length; specificIndex++) {
			const specificItem = specific[specificIndex];
			if (!specificItem || !specificItem.date) continue;

			// Create Date objects for FullCalendar
			const startDateTime = dayjs(`${specificItem.date}T${specificItem.startTime}:00`).toDate();
			const endDateTime = dayjs(`${specificItem.date}T${specificItem.endTime}:00`).toDate();
	...
	const handleEventClick = (clickInfo: EventClickArg) => {
		const { type, index } = clickInfo.event.extendedProps as { type: string; index: number };
		if (type === "recurring" && typeof index === "number") {
			// Show confirmation and delete recurring schedule
			if (confirm("Are you sure you want to delete this recurring schedule pattern?")) {
				removeRecurring({
					values: { index },
					params: { courseId },
				});
...
const AddScheduleModal = forwardRef<AddScheduleModalHandle, AddScheduleModalProps>(
	({ courseId }, ref) => {
		const [opened, setOpened] = useState(false);
		const { submit: addRecurring, isLoading: isLoadingRecurring } = useAddRecurringSchedule();
		const { submit: addSpecific, isLoading: isLoadingSpecific } = useAddSpecificDate();
			...
			validate: {
				date: (value, values) => {
					if (values.type === "specific") {
						...
						const dateStr = typeof value === 'string' ? value : dayjs(value).format('YYYY-MM-DD');
		...
		const toggleDay = (day: number) => {
			...
			if (current.includes(day)) {
				form.setFieldValue("daysOfWeek", current.filter((d) => d !== day));
			} else {
				form.setFieldValue("daysOfWeek", [...current, day].sort((a, b) => a - b));
			}
		};
				...
				<form onSubmit={handleSubmit}>
					<Stack gap="md">
							...
							<>
								<Input.Wrapper label="Days of Week" required withAsterisk>
									...
									{form.errors.daysOfWeek && (
										<div style={{ color: "var(--mantine-color-error)", fontSize: "var(--mantine-font-size-xs)", marginTop: "calc(var(--mantine-spacing-xs) / 2)" }}>
											{form.errors.daysOfWeek}
										</div>
									)}
								</Input.Wrapper>
								...
								<Group grow>
									<DateInput
										...
										onChange={(date) =>
											form.setFieldValue("startDate", date ? dayjs(date).format("YYYY-MM-DD") : undefined)
										}
										clearable
									/>
									<DateInput
										...
										onChange={(date) =>
											form.setFieldValue("endDate", date ? dayjs(date).format("YYYY-MM-DD") : undefined)
										}
										clearable
									/>
								</Group>
						...
						<Group justify="flex-end">
							<Button variant="subtle" onClick={() => setOpened(false)} disabled={isLoading} type="button">
								Cancel
							</Button>

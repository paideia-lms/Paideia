...
import { GradingConfigEditor, QuestionsList } from "./others";
...
export function RegularQuizBuilder({ form }: RegularQuizBuilderProps) {
	...
	// assert it is a regular quiz
	assertZodInternal(
		`RegularQuizBuilder: rawQuizConfig is a regular quiz but got ${JSON.stringify(_rawQuizConfig)}`,
		_rawQuizConfig,
		z.object({
...
function NestedQuizTab({ form, quizIndex }: NestedQuizTabProps) {
	...
	const nestedQuiz = _rawQuizConfig.nestedQuizzes[quizIndex];
...
export function ContainerQuizBuilder({ form }: ContainerQuizBuilderProps) {
	// type narrowing
	const _rawQuizConfig = form.getValues().rawQuizConfig;
	// assert it is a nest quiz
	assertZodInternal(
		"ContainerQuizBuilder: rawQuizConfig should be a container quiz",
		_rawQuizConfig,
		z.object({
			type: z.literal("container"),
		}),
	...
	const nestedQuizzes = useFormWatchValue(form, "rawQuizConfig.nestedQuizzes", {
		derived(value) {
			// only get the data that we need to display in the tabs
			return value.map((quiz) => ({
				id: quiz.id,
				title: quiz.title,
			}));
		},
	...
	// const gradingEnabled = useFormWatchValue(form, 'rawQuizConfig.grading.enabled', _rawQuizConfig.grading?.enabled ?? false);
	...
	return (
		<Stack gap="lg">
			<pre>{JSON.stringify(nestedQuizzes, null, 2)}</pre>
			<TextInput
				{...form.getInputProps("rawQuizConfig.title")}
				key={form.key("rawQuizConfig.title")}

...
import type { NestedQuizConfig, QuizAnswers, QuizConfig } from "./quiz-config.types";
import { isContainerQuiz } from "./quiz-config.types";
...
export function useNestedQuizState({
    ...
    const [currentNestedQuizId, setCurrentNestedQuizId] = useState<string | null>(null);
    const [completedQuizIds, setCompletedQuizIds] = useState<Set<string>>(new Set());
    const [submittedAnswers, setSubmittedAnswers] = useState<Record<string, QuizAnswers>>({});

    // Get the active nested quiz if one is selected
    const activeNestedQuiz = currentNestedQuizId && isContainerQuiz(quizConfig)
        ? quizConfig.nestedQuizzes?.find((q) => q.id === currentNestedQuizId) ?? null
        : null;
    ...
    const completionProgress = isContainerQuiz(quizConfig) && quizConfig.nestedQuizzes
        ? Math.round((completedQuizIds.size / quizConfig.nestedQuizzes.length) * 100)
        : 0;

...
export function GradingConfigEditor({ form }: GradingConfigEditorProps) {
    ...
    const currentConfig = form.state.values.rawQuizConfig?.grading || {
        enabled: false,
        passingScore: 70,
    ...
    // Helper to update grading config
    const updateGradingConfig = (updates: Partial<GradingConfig>) => {
        const newConfig = { ...currentConfig, ...updates };
        const quizConfig = form.state.values.rawQuizConfig;
        if (quizConfig) {
            form.setFieldValue('rawQuizConfig', { ...quizConfig, grading: newConfig });
        }
    };
        ...
        <Paper withBorder p="md" radius="md">
            <Stack gap="sm">
                ...
                <Checkbox
                    ...
                    onChange={(e) => updateGradingConfig({ enabled: e.currentTarget.checked })}
                />
                ...
                {currentConfig.enabled && (
                    <>
                        <NumberInput
                            ...
                            value={currentConfig.passingScore || 70}
                            onChange={(val) =>
                                updateGradingConfig({
                                    passingScore: typeof val === 'number' ? val : 70,
                                })
                            }
                            min={0}
                            max={100}
                        ...
                        <Checkbox
                            ...
                            checked={currentConfig.showScoreToStudent || false}
                            onChange={(e) =>
                                updateGradingConfig({ showScoreToStudent: e.currentTarget.checked })
                            }
                        />

                        <Checkbox
                            label="Show Correct Answers After Submission"
                            checked={currentConfig.showCorrectAnswers || false}
                            onChange={(e) =>
                                updateGradingConfig({ showCorrectAnswers: e.currentTarget.checked })
                            }
                        />
                    </>
                )}
...
// Nested quiz grading editor (for use with onChange pattern)
        ...
        <Paper withBorder p="md" radius="md">
            <Stack gap="sm">
                ...
                <Checkbox
                    ...
                    checked={currentConfig.enabled}
                    onChange={(e) =>
                        onChange({ ...currentConfig, enabled: e.currentTarget.checked })
                    }
                />
                    ...
                    <>
                        ...
                        <Checkbox
                            ...
                            onChange={(e) =>
                                onChange({
                                    ...currentConfig,
                                    showScoreToStudent: e.currentTarget.checked,
                                })
                            }
                        />
                        ...
                        <Checkbox
                            ...
                            onChange={(e) =>
                                onChange({
                                    ...currentConfig,
                                    showCorrectAnswers: e.currentTarget.checked,
                                })
                            }
                        />
                    </>
                    ...
                    {isExpanded && (
                        <Stack gap="sm">
                            ...
                            <Textarea
                                ...
                                onChange={(e) => onUpdate({ ...question, prompt: e.currentTarget.value })}
                                minRows={2}
                                required
                                size="sm"
                            />
                            ...
                            <Textarea
                                ...
                                onChange={(e) => onUpdate({ ...question, feedback: e.currentTarget.value })}
                                minRows={2}
                                size="sm"
                            />
    ...
    return (
        <Stack gap="md">
            ...
            {items.length === 0 ? (
                ...
                >
                    <SortableContext
                        items={items.map((item) => (item.type === 'question' ? item.data.id : item.id))}
                                            ...
                                            <Text fw={500} size="sm">
                                                ...
                                                {items
                                                    .slice(
                                                        ...
                                                        items.findIndex(
                                                            (i) =>
                                                                i.type === 'question' && i.data.id === activeItem.data.id,
                                                        ) + 1,
                                                    )
                                                    .filter((i) => i.type === 'question').length}
                                                :{' '}
...
export function RegularQuizBuilder({ form }: RegularQuizBuilderProps) {
    ...
    const handleItemsChange = (items: QuestionOrPageBreak[]) => {
        ...
        form.setFieldValue('rawQuizConfig', { ...config, pages });
    };
    ...
    return (
        <Stack gap="lg">
            <form.AppField name="rawQuizConfig.title">
                {(field) => (
                    <field.TextInputField label="Quiz Title" required />
                )}
            </form.AppField>
...
interface NestedQuizTabProps {
    ...
    onChange: (updated: NestedQuizConfig) => void;
}

function NestedQuizTab({ nestedQuiz, onChange }: NestedQuizTabProps) {
    // Convert pages structure to flat list with page breaks
    const itemsToList = (): QuestionOrPageBreak[] => {
        const result: QuestionOrPageBreak[] = [];
    ...
    return (
        <Stack gap="md">
            <TextInput
                label="Quiz Title"
                value={nestedQuiz.title}
                onChange={(e) => onChange({ ...nestedQuiz, title: e.currentTarget.value })}
            />

            <Textarea
                label="Description (optional)"
                value={nestedQuiz.description || ''}
                onChange={(e) => onChange({ ...nestedQuiz, description: e.currentTarget.value })}
                minRows={2}
            />
            ...
            <NumberInput
                ...
                value={nestedQuiz.globalTimer || undefined}
                onChange={(val) =>
                    onChange({
                        ...nestedQuiz,
            ...
            <NestedGradingConfigEditor
                config={nestedQuiz.grading}
                onChange={(grading) => onChange({ ...nestedQuiz, grading })}
            />
...
export function ContainerQuizBuilder({ form }: ContainerQuizBuilderProps) {
    ...
    const [activeTab, setActiveTab] = useState<string | null>(nestedQuizzes[0]?.id || null);
    ...
    const addNestedQuiz = () => {
        ...
        form.setFieldValue('rawQuizConfig', {
            ...config,
            nestedQuizzes: [...nestedQuizzes, newQuiz],
        });
        setActiveTab(newQuiz.id);
    };

    const updateNestedQuiz = (index: number, updated: NestedQuizConfig) => {
        const newNestedQuizzes = [...nestedQuizzes];
        newNestedQuizzes[index] = updated;
        form.setFieldValue('rawQuizConfig', { ...config, nestedQuizzes: newNestedQuizzes });
    };

    const removeNestedQuiz = (index: number) => {
        const newNestedQuizzes = nestedQuizzes.filter((_, i) => i !== index);
        form.setFieldValue('rawQuizConfig', { ...config, nestedQuizzes: newNestedQuizzes });
                        ...
                        {nestedQuizzes.map((quiz, index) => (
                            <Tabs.Panel key={quiz.id} value={quiz.id} pt="md">
                                <Stack gap="md">
                                    ...
                                    <NestedQuizTab
                                        nestedQuiz={quiz}
                                        onChange={(updated) => updateNestedQuiz(index, updated)}
                                    />
                                </Stack>
                            </Tabs.Panel>
                        ))}

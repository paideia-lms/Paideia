  ...
  if (categoryIdParam) {
    ...
    if (!Number.isNaN(idNum)) {
      ...
      if (catRes.ok) {
        ...
        const subRes = await tryFindSubcategories(payload, idNum);
        const totalRes = await tryGetTotalNestedCoursesCount(payload, idNum);
        const ancestorsRes = await tryGetCategoryAncestors(payload, idNum);
...
export default function AdminCategoriesPage({
  ...
  const { flat, selectedCategory } = loaderData;
  ...
  const tree = useTree<FlatNode>({
    ...
    onDrop: async (items, target) => {
      ...
      // Determine new parent id based on drop location
      ...
      await reorderCategories({
        ...
        newParentId: Number(newParent.substring(1)),
      });
    },
    indent: 20,
    dataLoader: {
      getItem: (id: string) => {
        if (id === "root") {
          return {
            ...
            name: "Root",
            parentId: null,
            children: Object.keys(flat).filter(
              (k) => flat[k].parentId === null,
            ),
      ...
      getChildren: (id: string) => {
        if (id === "root")
          return Object.keys(flat).filter((k) => flat[k].parentId === null);
      ...
      <Stack gap="lg">
        ...
        {selectedCategory ? (
          <Group align="flex-start" grow>
            ...
            <Box style={{ flex: 2 }}>{DetailsPanel}</Box>
          </Group>
        ) : (
          TreePanel
        )}
...
function EditCategoryModal({
  ...
  useEffect(() => {
    if (!opened) return;
    const initialParent = defaultParentId != null ? String(defaultParentId) : "";

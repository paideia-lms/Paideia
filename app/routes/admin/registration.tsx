...
const inputSchema = z.object({
    disableRegistration: z.preprocess(
        (val) => {
    ...
    showRegistrationButton: z.preprocess(
        (val) => {
            if (typeof val === "string") {
                return val === "true";
            }
...
export function useUpdateRegistrationConfig() {
    ...
    const update = (data: { disableRegistration: boolean; showRegistrationButton: boolean }) => {
        ...
        formData.set("disableRegistration", data.disableRegistration ? "true" : "false");
        formData.set("showRegistrationButton", data.showRegistrationButton ? "true" : "false");
...
export default function AdminRegistration({ loaderData }: Route.ComponentProps) {
    ...
    const { settings: { disableRegistration, showRegistrationButton } } = loaderData;
            ...
            <form method="post" onSubmit={form.onSubmit((values) => {
                ...
                <Stack gap="sm">
                    ...
                    {!form.values.disableRegistration && <Switch
                        {...form.getInputProps("showRegistrationButton", { type: "checkbox" })}

...
import { isPgDumpAvailable } from "server/utils/cli-dependencies-check";
...
export const action = async ({ request, context }: Route.ActionArgs) => {
	...
	// Check if pg_dump is available before attempting dump
	const pgDumpAvailable = await isPgDumpAvailable();
	if (!pgDumpAvailable) {
		return badRequest({
			error:
...
export default function MigrationsPage({ loaderData }: Route.ComponentProps) {
	const { statuses, pgDumpAvailable } = loaderData;
			...
			<Stack gap="lg">
				...
				>
					...
					<Button
						...
						disabled={!pgDumpAvailable || isLoading}
						loading={isLoading}
					>
						Dump Database
					</Button>
				...
				<Alert variant="light" color="blue" title="About Migrations">
					<Stack gap="xs">
						...
						{!pgDumpAvailable && (
							<div>
								<strong>Note:</strong> Database dump requires{" "}
								<code>pg_dump</code> to be installed. The <code>pg_dump</code>{" "}
								version must match or be compatible with your PostgreSQL server
								version.
							</div>
						)}
					</Stack>

...
function getResultUrl(
	result: Route.ComponentProps["loaderData"]["results"][number],
) {
	...
	switch (relationTo) {
		case "users":
			return getRouteUrl("/user/profile/:id?", {
				params: { id: result.doc.value.toString() },
			});
		case "courses":
			return getRouteUrl("/course/:courseId", {
				params: { courseId: result.doc.value.toString() },
				searchParams: {},
			});
		default:
			return "#";
...
export default function SearchPage({ loaderData }: Route.ComponentProps) {
	const { results, totalDocs, totalPages, currentPage, searchParams } =
		loaderData;
	const setQueryParams = useNuqsSearchParams(searchSearchParams);
												...
												<Group justify="space-between" align="flex-start">
													<Stack gap="xs" style={{ flex: 1 }}>
														<Group gap="xs">
															<Badge
																color={
																	collectionColors[result.doc.relationTo] ??
																	collectionColors.default
																}
																leftSection={
																	collectionIcons[result.doc.relationTo] ??
																	collectionIcons.default
																}
															>
																{collectionLabels[result.doc.relationTo] ??

...
import { IconBook, IconCalendar, IconClock, IconMapPin, IconUser, IconUsers } from "@tabler/icons-react";
...
function getMockCourseInstances(code: string) {
    ...
    return semesters.map((semester, semesterIndex) => {
        ...
        const isCurrent = semesterDate.isSame(currentDate, "month") ||
            (semesterDate.isBefore(currentDate) && semesterDate.add(4, "month").isAfter(currentDate));
        ...
        for (let i = 0; i < numSections; i++) {
            ...
            if (isPast) {
                ...
                status = semesterDate.diff(currentDate, "month") < 6 ? "open" : "future";
            }
...
export default function CatalogPage({ loaderData }: Route.ComponentProps) {
    ...
    return (
        <Container size="xl" py="xl">
            ...
            <meta property="og:description" content={`${courseInfo.code}: ${courseInfo.title} - Course catalog and sections`} />
            <Stack gap="xl">
                {/* Course Header */}

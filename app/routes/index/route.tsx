...
export const loaderSearchParams = {
	calendarDate: parseAsString.withDefault(""),
};

const createRouteLoaderWithParams = createRouteLoader({
		...
		const todaysCourseMeetings = [
			...
			userAccessContext.enrollments.map((enrollment, index) => {
				return {
					...
					startTime:
						enrollment.course.recurringSchedules?.[0]?.startTime ?? "00:00 AM",
					endTime:
						enrollment.course.recurringSchedules?.[0]?.endTime ?? "00:00 AM",
				...
				<Grid>
												...
												<Stack gap={4}>
													{course.startDate && (
														<Group gap={4}>
															...
															<Text size="xs" c="dimmed" lineClamp={1}>
																Start:{" "}
																{formatDateTimeForDisplay(course.startDate, {
																	style: "dayjs",
																})}
															</Text>
														</Group>
													...
													{course.endDate && (
														<Group gap={4}>
															...
															<Text size="xs" c="dimmed" lineClamp={1}>
																End:{" "}
																{formatDateTimeForDisplay(course.endDate, {
																	style: "dayjs",
																})}
															</Text>
														</Group>
					...
					<Grid.Col span={{ base: 12, lg: 4 }}>
						<Stack gap="md">
							{/* Calendar Widget */}
							<CalendarWidget
								sortedScheduleItems={sortedScheduleItems}
...
function PublicDashboard({
	...
	const { featuredCourses, showRegistrationButton, searchParams } = loaderData;

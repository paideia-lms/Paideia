import { Avatar, Badge, Container, Group, Tabs, Text, Title } from "@mantine/core";
    ...
    const handleTabChange = (value: string | null) => {
        ...
        switch (value) {
            case UserTab.Profile:
                navigate(href("/user/overview/:id?", { id: userIdParam ? userIdParam : undefined })
                );
                break;
            case UserTab.Preference:
                navigate(href("/user/edit/:id?", { id: userIdParam ? userIdParam : undefined })
                );
                break;
            case UserTab.Modules:
                navigate(href("/user/modules/:id?", { id: userIdParam ? userIdParam : undefined })
                );
                break;
            case UserTab.Grades:
                navigate(href("/user/grades/:id?", { id: userIdParam ? userIdParam : undefined })
                );
                break;
            case UserTab.Notes:
                navigate(href("/user/notes/:id?", { id: userIdParam ? userIdParam : undefined })
                );
                break;
        }
    };
            ...
            <div className={classes.header}>
                <Container size="xl" className={classes.mainSection}>
                    <Group justify="space-between">
                        <Group>
                            <Avatar src={user.avatarUrl} alt={fullName} size={48} radius={48} />
                            <div>
                                <Group gap="xs" mb={4}>
                                    <Title order={2}>
                                        {fullName}

...
export function ContainerQuizAttemptComponent() {
	const { quizConfig, submission, remainingTime, nestedQuizId } =
		useContainerQuizAttemptContext();
	...
	const activeNestedQuiz = nestedQuizId
		? (quizConfig.nestedQuizzes?.find((q) => q.id === nestedQuizId) ?? null)
		: null;
		...
		>
			<Stack gap="md">
				...
				{nestedQuizId === null ? (
					...
					>
						<RegularQuizAttemptComponent grading={quizConfig.grading} />
					</RegularQuizAttemptContextProvider>
				) : null}
			</Stack>
		</NestedQuizContextProvider>

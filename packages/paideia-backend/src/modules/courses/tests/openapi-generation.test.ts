...
} from "../api/course-activity-module-link-management";
...
describe("Activity Module Link API OpenAPI generation", () => {
	test("should include link paths in generated OpenAPI spec", async () => {
		const openApiGenerator = createOpenApiGenerator();
		const spec = await openApiGenerator.generate(linkApiRouter, {
			info: { title: "Paideia LMS API", version: "1.0.0" },
			servers: [{ url: "http://localhost:3001/openapi" }],
		});
		...
		const paths = spec.paths as Record<string, unknown>;
		expect(paths["/course-activity-module-links"]).toBeDefined();
		expect(paths["/course-activity-module-links/{linkId}"]).toBeDefined();
		expect(paths["/course-activity-module-links/by-course/{courseId}"]).toBeDefined();
		expect(paths["/course-activity-module-links/by-activity-module/{activityModuleId}"]).toBeDefined();
		expect(paths["/course-activity-module-links/search"]).toBeDefined();
		expect(paths["/course-activity-module-links/{linkId}/settings"]).toBeDefined();
		expect(paths["/course-activity-module-links/exists"]).toBeDefined();
	});

	test("POST /course-activity-module-links should have post method", async () => {
		const openApiGenerator = createOpenApiGenerator();
		const spec = await openApiGenerator.generate(linkApiRouter, {
			info: { title: "Paideia LMS API", version: "1.0.0" },
			servers: [{ url: "http://localhost:3001/openapi" }],
	...
	test("GET /course-activity-module-links/{linkId} should have correct method and path params", async () => {
		const openApiGenerator = createOpenApiGenerator();
		const spec = await openApiGenerator.generate(linkApiRouter, {
			info: { title: "Paideia LMS API", version: "1.0.0" },
			servers: [{ url: "http://localhost:3001/openapi" }],
		...
		const pathItem = (spec.paths as Record<string, unknown>)[
			"/course-activity-module-links/{linkId}"
		] as Record<string, unknown>;
		expect(pathItem).toBeDefined();
		expect(pathItem.get).toBeDefined();
		...
		expect(getOp.parameters).toBeDefined();
		const params = getOp.parameters as Array<Record<string, unknown>>;
		const linkIdParam = params.find((p) => p.name === "linkId");
		expect(linkIdParam).toBeDefined();
		expect(linkIdParam?.in).toBe("path");
	});

	test("DELETE /course-activity-module-links/{linkId} should have delete method", async () => {
		const openApiGenerator = createOpenApiGenerator();
		const spec = await openApiGenerator.generate(linkApiRouter, {
			info: { title: "Paideia LMS API", version: "1.0.0" },
			servers: [{ url: "http://localhost:3001/openapi" }],
	...
	test("GET /course-activity-module-links/search should support optional query params", async () => {
		const openApiGenerator = createOpenApiGenerator();
		const spec = await openApiGenerator.generate(linkApiRouter, {
			info: { title: "Paideia LMS API", version: "1.0.0" },
			servers: [{ url: "http://localhost:3001/openapi" }],
		...
		const pathItem = (spec.paths as Record<string, unknown>)[
			"/course-activity-module-links/search"
		] as Record<string, unknown>;
		expect(pathItem).toBeDefined();
		expect(pathItem.get).toBeDefined();
	});

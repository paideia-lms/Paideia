...
export async function deleteEverythingInBucket({
	logger,
}: {
	...
	try {
		...
		if (deleteResponse.Errors && deleteResponse.Errors.length > 0) {
			logger.error(
				`Some objects failed to delete: ${deleteResponse.Errors.map((error) => error.Key).join(", ")}`,
			);
		}
	} catch (error) {
		logger.error(
			`Error deleting objects from bucket: ${error instanceof Error ? error.message : String(error)}`,
		);
		throw error;
	}
}

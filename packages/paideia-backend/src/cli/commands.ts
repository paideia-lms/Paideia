...
import { deleteEverythingInBucket } from "server/modules/infrastructure/services/s3-client";
...
const cliRouter = {
	help: cliOs
		...
		.handler(async ({ context }) => {
			...
			context.payload.logger.info(`Paideia LMS - ${context.packageJson.version}`);
			displayHelp();
		}),
	migrate: {
		status: cliOs

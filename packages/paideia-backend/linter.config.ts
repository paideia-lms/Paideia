...
const astPatterns = {
	...
	exportConstActionWithoutImport: (
		...
		let hasImport = false;
		const visitForImport = (n: ts.Node): void => {
	...
	missingExportFunctionGetRouteUrl: (
		...
		const visitForExportFunction = (n: ts.Node): void => {
	...
	isHookImportedFromRoute: (
		...
		const visitForImports = (n: ts.Node): boolean => {
			if (ts.isImportDeclaration(n)) {
				...
				if (ts.isStringLiteral(moduleSpecifier)) {
					...
					if (
						modulePath.startsWith("apps/paideia/app/routes/") ||
	...
	multipleHooksInComponent: (
		...
		const visitForHookDeclarations = (n: ts.Node): void => {
		...
		if (!cacheEntry) {
			...
			const findDefaultExport = (n: ts.Node): void => {
				if (
			...
			const countHooksInComponent = (n: ts.Node): void => {
	...
	tooManyUseStateHooks: (
		...
		if (!cacheEntry) {
			...
			const countUseStateInComponent = (n: ts.Node): void => {
	...
	missingActionInMap: (
		...
		const collectActions = (n: ts.Node): void => {
...
export const rules: LintRule[] = [
	{
		...
		includes: ["apps/paideia/server/**/*.ts", "packages/paideia-backend/src/**/*.ts"],
	...
	},
	{
		...
		includes: ["packages/paideia-backend/src/internal/**/*.ts", "!packages/paideia-backend/src/internal/**/*.test.ts", "!packages/paideia-backend/src/internal/**/*.spec.ts"],
		mode: "ast", // Use AST for more accurate detection (ignores comments/strings)
		astPatterns: [
			{
	...
	},
	{
		...
		includes: ["apps/paideia/server/**/*.ts", "packages/paideia-backend/src/**/*.ts", "!apps/paideia/server/**/*.test.ts", "!apps/paideia/server/**/*.spec.ts", "!packages/paideia-backend/src/**/*.test.ts", "!packages/paideia-backend/src/**/*.spec.ts"],
		mode: "ast", // Use AST for more accurate detection (ignores comments/strings)
		astPatterns: [
			{

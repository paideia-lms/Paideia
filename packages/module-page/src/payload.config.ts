...
const pg = postgresAdapter({
	...
	// Use migrations only (push: false) to avoid interactive schema prompts when
	// DB has tables from other modules (e.g. courses.pages). Tests use migrateFresh.
	push: false,
});
...
const config = buildConfig({
	...
	collections: [...UserModule.collections, Pages],
	admin: {},
	defaultDepth: 1,
	typescript: {

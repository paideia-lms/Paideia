...
import { MOCK_INFINITY } from "../utils/constants";
import { Result } from "typescript-result";
import { InvalidArgumentError, UnknownError, DevelopmentError } from "@paideia/shared";
import {
	MediaInUseError,
...
import * as schemas from "../payload-generated-schema";
import { and, eq, sql } from "@payloadcms/db-postgres/drizzle";
import {
	getAllMediaFields,
	mapMediaFieldsToDrizzle,
} from "../internal/utils/get-all-media-fields";
	...
	return Result.try(
		async () => {
			...
			return transactionInfo.tx(async () => {
				...
				const bucket =
					envVars.S3_BUCKET.value ?? envVars.S3_BUCKET.default ?? "";
				await Promise.all(
					foundMedia
...
/**
 * Union type of table names in the module-user schema.
 * Used for type-safe schema lookups in tryFindMediaUsages.
 */
type KeysOfTables = Extract<
	keyof typeof schemas,
	"users" | "media" | "users_sessions"
>;
		...
		async () => {
			...
			const simpleQueries = simpleDrizzleFields.map((field) =>
				payload.db.drizzle
					...
					.where(
						eq(
							(schemas[field.tableRef as KeysOfTables] as Record<string, unknown>)[
			...
			const nestedQueries = nestedDrizzleFields.map((field) => {
				...
				const joinTable = schemas[joinTableRef] as {
					_order: unknown;
					_parentID: unknown;
					[key: string]: unknown;
				};
				...
				return payload.db.drizzle
					...
					.from(parentTable)
					.innerJoin(
						joinTable as typeof parentTable,
						eq(parentTable.id, joinTable._parentID as Parameters<typeof eq>[1]),
					)
					.where(
						eq(
							joinTable[field.schemaFieldName] as Parameters<typeof eq>[0],
							mediaId,
						),
					);
			});
			...
			const arrayQueries = arrayDrizzleFields.map((field) => {
				...
				return payload.db.drizzle
					...
					.innerJoin(
						joinTable as typeof parentTable,
						eq(parentTable.id, joinTable.parent as Parameters<typeof eq>[1]),
					)
					.where(
						and(
							eq(
								joinTable.mediaID as Parameters<typeof eq>[0],
								mediaId,
							),
							eq(
								joinTable.path as Parameters<typeof eq>[0],
								field.fieldPath,
							),
						),
					);
			...
			let combinedQuery = allQueries[0]! as { unionAll: (q: unknown) => unknown };
			for (let i = 1; i < allQueries.length; i++) {
				combinedQuery = combinedQuery.unionAll(allQueries[i]!) as typeof combinedQuery;
			}

			const rawResult = await combinedQuery;
			const allUsagesResult = rawResult as unknown as Array<{
				collection: string;
				documentId: number;
				fieldPath: string;
			}>;

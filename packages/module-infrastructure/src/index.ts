...
// export { envVars, validateEnvVars };
...
import { deleteEverythingInBucket, s3Client } from "./services/s3-client";
...
import { getMigrationStatus } from "./services/migration-status";
...
export class InfrastructureModule {
    ...
    async migrateFresh(opts?: { migrations: Migration[]; forceAcceptWarning?: boolean }) {
        return migrateFresh({
            payload: this.payload,
            migrations: opts?.migrations ?? [],
            forceAcceptWarning: opts?.forceAcceptWarning ?? false,
        });
    }

    async migrateStatus(opts?: { migrations: Migration[] }) {
    ...
    async migrateUp(opts?: { migrations: Migration[] }) {
        return this.payload.db.migrate({
            migrations: opts?.migrations ?? [],
        });
    }

    async cleanS3() {
        return deleteEverythingInBucket({
            logger: this.payload.logger,
        });
    }
}

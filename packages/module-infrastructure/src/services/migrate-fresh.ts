...
async function getTransaction(adapter: Payload["db"], req: PayloadRequest) {
	...
	const sessions = (
		adapter as {
			sessions?: Record<string | number, { db: typeof adapter.drizzle }>;
		}
	).sessions;
	if (!sessions) {
		return adapter.drizzle;
...
export async function migrateFresh({
	...
	if (forceAcceptWarning === false) {
		const acceptWarning = await confirm(
			{
				default: false,
			...
			if (error instanceof ExitPromptError || error instanceof Error && error.name === "ExitPromptError") {
				payload.logger.error("User cancelled migration. Exiting...");
				process.exit(0);
			}
		});
		if (!acceptWarning) {
	...
	for (const migration of migrations) {
		...
		try {
			...
			payload.logger.error({
				...
				msg: parseError(
					err,
					`Error running migration ${migration.name}. Rolling back`,
				),
			});
			process.exit(1);
		}
